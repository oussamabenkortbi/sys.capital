...
const Team = () => {
  return (
    <section className="bg-white dark:bg-black py-24 px-6 max-w-5xl mx-auto text-black dark:text-white select-text">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-16 text-center text-xl font-light tracking-wide">
        {teamMembers.map(({ name, role }, i) => (
          <div key={i} className="space-y-2">
            <p className="uppercase font-black tracking-widest text-3xl">{name}</p>
            <p className="uppercase opacity-60 dark:opacity-70 tracking-wide">{role}</p>
          </div>
        ))}
      </div>
    </section>
