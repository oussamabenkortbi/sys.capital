...
const FeaturedProducts = () => {
  ...
  return (
    <section id="products" className="relative py-24 sm:py-28 md:py-32 px-6 sm:px-8 select-text overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto space-y-16 sm:space-y-20 md:space-y-24">
        ...
        <div className="text-center space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto font-light leading-relaxed font-inter">
              Premium business equipment designed for
                ...
                <CardContent className="relative flex flex-col lg:flex-row justify-between items-center gap-8 sm:gap-12 p-8 sm:p-12 lg:p-16">
                      ...
                      <h3 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tighter leading-[0.9]">
                        <GradientText className="bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900 dark:from-white dark:via-slate-200 dark:to-white bg-clip-text text-transparent group-hover:from-indigo-600 group-hover:via-purple-600 group-hover:to-violet-600 transition-all duration-500">
                          {name.split(' ').map((word, index) => (
                            <span key={index} className="inline-block mr-4 hover:animate-pulse">
                              {word}
                            </span>
                          ))}
                        </GradientText>
                  ...
                  <div className="flex-1 text-center lg:text-right space-y-6 max-w-lg">
                    <div className="space-y-3">
                      ...
                      {/* Currency highlight */}
                      <p className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest font-mono">
                        Algerian Dinar
                      </p>
                    </div>
                        ...
                        <p className="text-base sm:text-lg font-light text-slate-600 dark:text-slate-400 leading-relaxed font-inter max-w-md mx-auto lg:ml-auto lg:mr-0">
                          {details.split(' | ').map((detail, index, array) => (
                            <span key={index} className="block sm:inline">
                              <span className="font-medium text-slate-800 dark:text-slate-200">{detail}</span>
                              {index < array.length - 1 && <span className="hidden sm:inline mx-3 text-indigo-400">•</span>}
                            </span>
                          ))}
                        </p>
        ...
        <div className="text-center space-y-6 pt-8">
          <div className="relative inline-block group">
            <Button asChild size="lg" className="relative overflow-hidden px-8 py-4 rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 hover:from-indigo-500 hover:via-purple-500 hover:to-violet-500 border-0 shadow-xl hover:shadow-2xl transform transition-all duration-300 hover:scale-105">
              <a href="#store" className="inline-flex items-center gap-3 text-white font-semibold text-lg tracking-wide relative z-10">
                <div className="relative">
                  <Store className="w-5 h-5 transition-transform duration-300 group-hover:rotate-12 group-hover:scale-110" aria-hidden="true" />
                  <div className="absolute inset-0 blur-sm opacity-50 group-hover:opacity-100 transition-opacity duration-300">
                    <Store className="w-5 h-5" aria-hidden="true" />
                  </div>
                </div>
                <span className="font-inter">Visit Store</span>
              </a>
            </Button>
            {/* Button glow effect */}
