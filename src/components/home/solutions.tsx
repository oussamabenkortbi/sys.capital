const Solutions = () => {
  ...
  return (
    <section id="software" className="bg-white dark:bg-black min-h-screen px-6 py-32 text-black dark:text-white select-text">
      <div className="max-w-6xl mx-auto text-center space-y-32">
        <h2 className="text-8xl font-extrabold uppercase tracking-tight leading-none">
          OUR <br /> SOFTWARE <br /> SOLUTIONS
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-24 uppercase font-black tracking-widest text-4xl md:text-5xl">
          {solutions.map(({ name, tagline, url }) => (
            ...
            >
              ...
              <div className="text-xl font-thin tracking-normal opacity-60 dark:opacity-70">{tagline}</div>
              <div className="mt-4 text-sm font-light tracking-wide text-gray-600 dark:text-gray-400 lowercase">{url}</div>
            </a>
          ))}
        </div>
      </div>
